 ...
 * A tracker asset.
 *
 * TODO - Add method to return the mime-type/content-type.TODO - Add method to return the accept-type.TODO - Check disk based/off-line storage (local).TODO - Serialize Queue for later submission (using queue.ToList()).TODO - Prevent csv/xml/json from net storage and xapi from local storage.
 */
public class TrackerAsset extends BaseAsset {
     ...
     *
     * NOTE: This regex handles matching brackets by using balancing groups. This should be tested in Mono if it works there too.
     ...
     * it's correctly typed backing field 'settings' instead.  This property should go into each asset having Settings of its own.    The actual class used should be derived from BaseAsset (and not directly from
     * ISetting).
     * The settings.
     */
    ...
    public void setSettings(ISettings value) {
        settings = (value instanceof TrackerAssetSettings ? (TrackerAssetSettings) value : (TrackerAssetSettings) null);
    }
     ...
     *
     *  @param anonymousId  The playerId of the anonymous player
     *  @return
     * true if it succeeds, false if it fails.
     */
     ...
     *
     *  @param trace A comma separated string with the values of the trace
     */
     ...
     *
     *  @param values Values of the trace.
     */
    public void trace(String... values) throws Exception {
        ...
        for (int i = 0; i < values.length; i++) {
            if (!getUtils().check(values[i], "Tracker: Trace param " + i + " is null or empty, ignoring trace.", "Tracker: Trace param " + i + " is null or empty", TraceException.class))
                return;
     ...
     *
     */
    public void actionTrace(String verb, String target_type, String target_id) throws Exception {
        boolean trace = true;
        trace &= getUtils().check(verb, "Tracker: Trace verb can't be null, ignoring. ", "Tracker: Trace verb can't be null.", TraceException.class);
        trace &= getUtils().check(target_type, "Tracker: Trace Target type can't be null, ignoring. ", "Tracker: Trace Target type can't be null.", TraceException.class);
        trace &= getUtils().check(target_id, "Tracker: Trace Target ID can't be null, ignoring. ", "Tracker: Trace Target ID can't be null.", TraceException.class);
        if (trace) {
     ...
     * true if it succeeds, false if it fails.
     */
    private RequestResponse issueRequest(String path, String method, Map<String, String> headers, String body) {
     ...
     * true if it succeeds, false if it fails.
     */
    private RequestResponse issueRequest(String path, String method, Map<String, String> headers, String body, Integer port) {
        ...
        if (ds != null) {
            ...
            /*RefSupport<RequestResponse> refVar___0 = new RefSupport<RequestResponse>();
    ...
    private void processQueue() throws XApiException {
        ...
        if (queue.getCount() > 0 || tracesPending.size() > 0 || tracesUnlogged.size() > 0) {
            ...
            if (getActive()) {
                if (sendUnloggedTraces()) {
                    ...
                    if ((!sendPendingTraces() || !(queue.getCount() > 0 && sendTraces(data))) && queue.getCount() > 0)
            ...
            if (settings.getBackupStorage()) {
                ...
                if (queue.getCount() > 0) {
                    ...
                    if (append_storage != null) {
                        append_storage.Append(settings.getBackupFile(), rawData);
                    } else if (storage != null) {
                        String previous = storage.exists(settings.getBackupFile()) ? storage.load(settings.getBackupFile()) : "";
                        if (storage.exists(settings.getBackupFile()))
                            storage.save(settings.getBackupFile(), previous + rawData);
                        else
    ...
    List<TrackerEvent> collectTraces() {
        Integer cnt = settings.getBatchSize() == 0 ? Integer.MAX_VALUE : settings.getBatchSize();
        cnt = Math.min(queue.getCount(), cnt);
        List<TrackerEvent> traces = Arrays.asList(queue.peek(cnt));
        return traces;
    }

    String processTraces(List<TrackerEvent> traces, TraceFormats format) throws XApiException {
        String data = "";
    ...
    boolean sendUnloggedTraces() throws XApiException {
        if (tracesUnlogged.size() > 0 && this.getActorObject() != null) {
            String data = processTraces(tracesUnlogged, settings.getTraceFormat());
    ...
    boolean sendTraces(String data) {
        switch (settings.getStorageType()) {
            case local:
                ...
                IAppend append_storage = getInterface(IAppend.class);
                if (storage != null) {
                    String previous = storage.exists(settings.getLogFile()) ? storage.load(settings.getLogFile()) : "";
     ...
     *
     *  @param score Score, (Recomended between 0 and 1)
     */
     ...
     *
     *  @param completion If set to
     *  {@code true}
     *  the trace action has been completed.
     */
     ...
     *
     *  @param progress Progress. (Recomended between 0 and 1)
     */
    ...
    public void setPosition(float x, float y, float z) throws Exception {
        ...
        addExtension(Extension.Position.toString().toLowerCase(), "{\"x\":" + x + ", \"y\": " + y + ", \"z\": " + z + "}");
    }
    ...
    public void setHealth(float health) throws Exception {
        if (getUtils().check(health, "Tracker: Health cant be null, ignoring.", "Tracker: Health cant be null.", ValueExtensionException.class))
     ...
     *  @param value Value.
     */
    public void setVar(String id, Dictionary<String, Boolean> value) throws Exception {
    ...
    public static class TrackerEvent {
        ...
        private static Map<String, String> getVerbIDs() {
            if (verbIds == null) {
                ...
                verbIds.put(TrackerAsset.Verb.Initialized.toString().toLowerCase(), "http://adlnet.gov/expapi/verbs/initialized");
                verbIds.put(TrackerAsset.Verb.Progressed.toString().toLowerCase(), "http://adlnet.gov/expapi/verbs/progressed");
                verbIds.put(TrackerAsset.Verb.Completed.toString().toLowerCase(), "http://adlnet.gov/expapi/verbs/completed");
                verbIds.put(TrackerAsset.Verb.Accessed.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/verbs/accessed");
                verbIds.put(TrackerAsset.Verb.Skipped.toString().toLowerCase(), "http://id.tincanapi.com/verb/skipped");
                verbIds.put(TrackerAsset.Verb.Selected.toString().toLowerCase(), "https://w3id.org/xapi/adb/verbs/selected");
                verbIds.put(TrackerAsset.Verb.Unlocked.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/verbs/unlocked");
                verbIds.put(TrackerAsset.Verb.Interacted.toString().toLowerCase(), "http://adlnet.gov/expapi/verbs/interacted");
                verbIds.put(TrackerAsset.Verb.Used.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/verbs/used");
        ...
        private static Map<String, String> getObjectIDs() {
            if (objectIds == null) {
                ...
                // Completable
                objectIds.put(CompletableTracker.Completable.Game.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/serious-game");
                objectIds.put(CompletableTracker.Completable.Session.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/session");
                objectIds.put(CompletableTracker.Completable.Level.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/level");
                objectIds.put(CompletableTracker.Completable.Quest.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/quest");
                objectIds.put(CompletableTracker.Completable.Stage.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/stage");
                objectIds.put(CompletableTracker.Completable.Combat.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/combat");
                objectIds.put(CompletableTracker.Completable.StoryNode.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/story-node");
                objectIds.put(CompletableTracker.Completable.Race.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/race");
                objectIds.put(CompletableTracker.Completable.Completable.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/completable");

                // Acceesible
                objectIds.put(AccessibleTracker.Accessible.Screen.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/screen");
                objectIds.put(AccessibleTracker.Accessible.Area.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/area");
                objectIds.put(AccessibleTracker.Accessible.Zone.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/zone");
                objectIds.put(AccessibleTracker.Accessible.Cutscene.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/cutscene");
                objectIds.put(AccessibleTracker.Accessible.Accessible.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/accessible");

                // Alternative
                objectIds.put(AlternativeTracker.Alternative.Question.toString().toLowerCase(), "http://adlnet.gov/expapi/activities/question");
                objectIds.put(AlternativeTracker.Alternative.Menu.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/menu");
                objectIds.put(AlternativeTracker.Alternative.Dialog.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/dialog-tree");
                objectIds.put(AlternativeTracker.Alternative.Path.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/path");
                objectIds.put(AlternativeTracker.Alternative.Arena.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/arena");
                objectIds.put(AlternativeTracker.Alternative.Alternative.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/alternative");

                // GameObject
                objectIds.put(GameObjectTracker.TrackedGameObject.Enemy.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/enemy");
                objectIds.put(GameObjectTracker.TrackedGameObject.Npc.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/non-player-character");
                objectIds.put(GameObjectTracker.TrackedGameObject.Item.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/item");
                objectIds.put(GameObjectTracker.TrackedGameObject.GameObject.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/activity-types/game-object");
            }
        ...
        private static Map<String, String> getExtensionIDs() {
            if (extensionIds == null) {
                ...
                extensionIds.put(TrackerAsset.Extension.Health.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/extensions/health");
                extensionIds.put(TrackerAsset.Extension.Position.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/extensions/position");
                extensionIds.put(TrackerAsset.Extension.Progress.toString().toLowerCase(), "https://w3id.org/xapi/seriousgames/extensions/progress");
            }
        ...
        public void setEvent(TrackerAsset.TrackerEvent.TraceVerb value) throws Exception {
            this.verb = value;
        ...
        public String toCsv() {
            return this.getTimeStamp()
                    + "," + getEvent().toCsv()
                    + "," + getTarget().toCsv()
                    + (this.getResult() == null || IsNullOrEmpty(this.getResult().toCsv()) ? "" : this.getResult().toCsv());
        }
        ...
        public String toJson() throws XApiException {
            ...
            json.put("actor", (getTracker().getActorObject() == null) ? new HashMap<>() : getTracker().getActorObject());
            ...
            DateFormat formatter = new SimpleDateFormat("yyyy-MM-ddTHH:mm:ss.fffZ");
         ...
         */
        public String toXml() {
            return ""; //"<TrackEvent \"timestamp\"=\"" + this.getTimeStamp().ToString(TimeFormat) + "\"" + " \"event\"=\"" + verbIds[this.getEvent().ToString().ToLower()] + "\"" + " \"target\"=\"" + this.getTarget() + "\"" + (this.getResult() == null || String.IsNullOrEmpty(this.getResult().toXml()) ? " />" : "><![CDATA[" + this.getResult().toXml() + "]]></TrackEvent>");
        }
        ...
        public static class TraceObject {
            ...
            public void setType(String value) throws Exception {
                if (getParent() == null || getParent().getTracker().getUtils().check(value, "xAPI Exception: Target Type is null or empty. Ignoring.", "xAPI Exception: Target Type can't be null or empty.", TargetXApiException.class))
                    _type = value;
            }
            ...
            public void setID(String value) throws Exception {
                if (getParent() == null || getParent().getTracker().getUtils().check(value, "xAPI Exception: Target ID is null or empty. Ignoring.", "xAPI Exception: Target ID can't be null or empty.", TargetXApiException.class))
                    _id = value;
            ...
            public String toCsv() {
                return getType().replace(",", "\\,") + "," + getID().replace(",", "\\,");
            }
            ...
            public Map<String, Object> toJson() throws TargetXApiException {
                ...
                if (getObjectIDs().containsKey(getType())) {
                    ...
                    throw (new TargetXApiException("Tracker-xAPI: Unknown definition for target type: " + getType()));
                } else {
                    getParent().getTracker().Log(Severity.Warning, "Tracker-xAPI: Unknown definition for target type: " + getType());
                }

                Map<String, Object> obj = new HashMap<>(), definition = new HashMap<>();

                obj.put("id", ((getParent().getTracker().getActorObject() != null) ? getParent().getTracker().ObjectId : "") + getID());
            ...
            public boolean isValid() throws Exception {
                return TrackerAssetUtils.quickCheck(getType()) && TrackerAssetUtils.quickCheck(getID());
            }
            ...
            public void setsVerb(String value) throws TrackerException {
                ...
                if (TrackerAssetUtils.ParseEnum(value, rv, Verb.class)) {
                    ...
                    if (getParent().getTracker().getStrictMode()) {
                        ...
                        getParent().getTracker().Log(Severity.Warning, "Tracker-xAPI: Unknown definition for verb: " + value);
                    }
        ...
        public static class TraceResult {
            ...
            public void setResponse(String value) throws Exception {
                if (getParent() == null || getParent().getTracker().getUtils().check(value, "xAPI extension: response Empty or null. Ignoring", "xAPI extension: response can't be empty or null", ValueExtensionException.class))
                    res = value;
            ...
            public void setScore(float value) throws Exception {
                if (getParent() == null || getParent().getTracker().getUtils().check(value, "xAPI extension: score null or NaN. Ignoring", "xAPI extension: score can't be null or NaN.", ValueExtensionException.class))
                    score = value;
            ...
            public void setExtensions(Map<String, Object> value) throws Exception {
            ...
            public String toCsv() {
                String result = ((success > -1) ? ",success" + intToBoolString(success) : "")
                        + ((completion > -1) ? ",completion" + intToBoolString(completion) : "")
                        + ((!IsNullOrEmpty(getResponse())) ? ",response," + getResponse().replace(",", "\\,") : "")
                        + ((!Float.isNaN(score)) ? ",score," + Float.toString(score).replace(',', '.') : "");
                    ...
                    while (it.hasNext()) {
                        ...
                        if (extension.getValue() != null) {
                            if (extension.getValue().getClass() == String.class) {
                                ...
                                Iterator it2 = ((HashMap<String, Object>) extension.getValue()).entrySet().iterator();

                                while (it2.hasNext()) {
                                    Map.Entry<String, Object> t = (Map.Entry<String, Object>) it2.next();
                                    smap += t.getKey() + "=" + t.getValue().toString().toLowerCase() + "-";
                                }
            ...
            public boolean isValid() throws Exception {
                ...
                if (getExtensions() != null && getExtensions().size() > 0) {
                    ...
                    while (it.hasNext()) {
                        Map.Entry<String, Object> extension = (Map.Entry<String, Object>) it.next();
                        valid &= TrackerAssetUtils.quickCheckExtension(extension.getKey(), extension.getValue());
                    }
                }
