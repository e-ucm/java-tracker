 ...
 * 
 * 1) Singleton (enum based) pattern implementation however is thread-safe,
 * compiler enforced and correct. 2) assets Map was changed into a ConcurrentMap
 * (untested however). 3) idGenerator is not thread safe.
 * 
...
public enum AssetManager {
	...
	public String registerAssetInstance(final IAsset asset, final String clazz) {
		...
		Log(Severity.Verbose, "Registering Asset %s/%s as %s",
				asset.getClassName(), clazz, id);
	...
	public String getVersionAndDependenciesReport() {
		...
		for (Map.Entry<String, IAsset> e : this.assets.entrySet()) {
			...
			String artifact = String.format("%s v%s", asset.getClassName(),
					asset.getVersion());
			report.append(padRight(artifact, col1w - artifact.length()));

			int cnt = 0;
			for (Map.Entry<String, String> dependency : asset.getDependencies()
					.entrySet()) {
				...
				if (low != null) {
					...
					report.append(
							String.format("| %s v%s [%s]", dependency.getKey(),
									dependency.getValue(), found ? "resolved"
											: "missing")).append(eol);
				} else {
					report.append("error");
				}
			...
			if (cnt == 0) {
				report.append(String.format("| %s", "No dependencies")).append(
						eol);
			}
		}
	...
	// / </summary>
	// / <remarks>Used for cleaning up in test suites (as static readonly
	// _instance member cannot be destroyed).</remarks>
